...
module decode(
    ...
    // Execute input 
    output reg [14:0]   control_bits_reg_o, // {branch_dest_for_alu_out, mem_load_unsigned, mem_strb_en[1:0], swap_baddr_alures, branch_en_invert, branch_enable, immd_en, wb_en, mw_en, mr_en, alu_op[3:0]}
    ...
    always @(*) begin
        ...
        endcase
        // Memory read enable
        lut_rd[4] = (instr_reg_i[6:0] == `OPCODE_LOAD);
        // Memory write enable
        lut_rd[5] = (instr_reg_i[6:0] == `OPCODE_STORE);
        // Write-back enable
        lut_rd[6] = (instr_reg_i[6:0] == `OPCODE_INT_I) || (instr_reg_i[6:0] == `OPCODE_INT_R) || (instr_reg_i[6:0] == `OPCODE_LOAD) || 
                    (instr_reg_i[6:0] == `OPCODE_JAL) || (instr_reg_i[6:0] == `OPCODE_JALR) || (instr_reg_i[6:0] == `OPCODE_LUI) ||
                    (instr_reg_i[6:0] == `OPCODE_AUIPC) ;
        // Immediate enable for ALU
        lut_rd[7] = (instr_reg_i[6:0] == `OPCODE_INT_I) || (instr_reg_i[6:0] == `OPCODE_LOAD) || (instr_reg_i[6:0] == `OPCODE_STORE) ||
                    (instr_reg_i[6:0] == `OPCODE_JALR) || (instr_reg_i[6:0] == `OPCODE_LUI);
        // branch enable
        lut_rd[8] = (instr_reg_i[6:0] == `OPCODE_BRANCH) || (instr_reg_i[6:0] == `OPCODE_JALR) || (instr_reg_i[6:0] == `OPCODE_JAL);
        // branch condition invert enable
        lut_rd[9] = instr_reg_i[12];
        // Swap alu result and pc_p4 in execute
        lut_rd[10] = (instr_reg_i[6:0] == `OPCODE_JALR);
        // mem_strb_en: log2(bytes used in load/store ops)
        lut_rd[12:11] = instr_reg_i[13:12];
        // mem_load_unsigned: dont sign extend load value
        lut_rd[13] = instr_reg_i[14];
        // branch_dest_for_alu_out
        lut_rd[14] = (instr_reg_i[6:0] == `OPCODE_AUIPC);
        ...
        case (instr_reg_i[6:0])
            ...
            `OPCODE_LUI     : lut_immd_type[2:0] = 3'd6; // u-type
            `OPCODE_AUIPC   : lut_immd_type[2:0] = 3'd6; // u-type
    ...
    always @(*) begin
        rs1_addr_used = (   (instr_reg_i[6:0] == `OPCODE_JAL)   ||
                            (instr_reg_i[6:0] == `OPCODE_LUI)   ||
                            (instr_reg_i[6:0] == `OPCODE_AUIPC)   ) ? (5'd0) : (instr_reg_i[19:15]);

        rs2_addr_used = (   (instr_reg_i[6:0] == `OPCODE_INT_R) ||
                            (instr_reg_i[6:0] == `OPCODE_STORE)  ||
                            (instr_reg_i[6:0] == `OPCODE_BRANCH)  ) ? (instr_reg_i[24:20]) : (5'd0);
    end
    ...
    assign u_encode_immd        = { instr_reg_i[31:12], 12'b0};                                                              // U-type encoding
